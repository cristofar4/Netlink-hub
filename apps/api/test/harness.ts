...
export async function createTestHarness(): Promise<TestHarness> {
  ...
  const reset = async () => {
    ...
    await prisma.$executeRawUnsafe(
      'TRUNCATE TABLE audit_events, refresh_tokens, challenges, request_nonces, power_commands, agent_enrollment_tokens, resources, data_usage_events, data_allocations, data_pools, agents, space_members, invitations, spaces, devices, users RESTART IDENTITY CASCADE',
    );
    mail.clear();
