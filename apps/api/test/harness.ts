...
export async function createTestHarness(): Promise<TestHarness> {
  ...
  const reset = async () => {
    ...
    await prisma.$executeRawUnsafe(
      'TRUNCATE TABLE rate_limit_buckets, account_guards, audit_events, refresh_tokens, challenges, request_nonces, remote_signals, remote_sessions, power_commands, file_transfers, print_jobs, agent_enrollment_tokens, resources, data_usage_events, data_allocations, data_pools, agents, space_members, invitations, spaces, devices, users RESTART IDENTITY CASCADE',
    );
    mail.clear();
